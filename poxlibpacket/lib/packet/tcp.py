...
#        print "setflagtcp",flag,self.flags,value
#        self.flags = (self.flags & ~flag) | (flag if value else 0)
    ...
    def parse(self, raw):
        ...
        self.parsed = True
        #print "flags in parse {:b} {}".format(self.flags,type(self.flags))
    ...
    def getflags(self):
        flags  =""
        if self.flags & tcp.SYN_flag:
            flags +="S"
        if self.flags & tcp.ACK_flag:
            flags +="A"
        if self.flags & tcp.FIN_flag:
            flags +="F"
        if self.flags & tcp.RST_flag:
            flags +="R"
        if self.flags & tcp.PSH_flag:
            flags +="P"
        if self.flags & tcp.URG_flag:
            flags +="U"
        if self.flags & tcp.ECN_flag:
            flags +="E"
        if self.flags & tcp.CWR_flag:
            flags +="C"
